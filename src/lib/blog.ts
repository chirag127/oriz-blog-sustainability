...
/**
 * Map a raw category string → one of five sustainability desks.
 * The underlying vertical keys (dev/finance/culture/health/personal) are kept
 * so the marker-colour CSS (--v-*) and every consuming component compile
 * unchanged; only the labels + routing regex are sustainability-specific.
 *   dev → Kitchen · finance → Repair · culture → Verify · health → Grow · personal → Field
 */
export function verticalFor(category: string): Vertical {
  const c = category.toLowerCase()
  if (/(grow|garden|soil|compost|plant|seed|season|home)/.test(c)) return 'health'
  if (/(repair|mend|fix|reuse|reduce|waste|zero-waste|circular)/.test(c)) return 'finance'
  if (/(verify|claim|greenwash|label|myth|science|evidence|research)/.test(c)) return 'culture'
  if (/(kitchen|food|pantry|cook|scrap|leftover|low-waste)/.test(c)) return 'dev'
  return 'personal'
}
